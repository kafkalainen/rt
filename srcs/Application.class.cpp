...
{
    t_queueFamilyIndices indices = findQueueFamilies(m_physicalDevice);
...
{
    t_swapChainSupportDetails swapChainSupport = querySwapChainSupport(m_physicalDevice);
...
{
    auto vertShaderCode = readFile("spirv/shader.vert.spv");
    auto fragShaderCode = readFile("spirv/shader.frag.spv");
...
{
    t_queueFamilyIndices queueFamilyIndices = findQueueFamilies(m_physicalDevice);
...
bool    Application::checkValidationLayerSupport()
{
    ...
    for (const char* layerName : validationLayers)
    {
        ...
        {
            if (kaf_strcmp(layerName, layerProperties.layerName) == 0)
            {
                layerFound = true;
                break;
            }
